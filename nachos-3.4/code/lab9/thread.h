...
// while executing kernel code.
  ...
  public:
    void SaveUserState();		// save user-level register state 保存用户寄存器
    void RestoreUserState();		// restore user-level register state 恢复用户寄存器
    void Terminated(); //进程结束后的处理
    int Join(int SpaceId); // 等待spaceId为SpaceId的进程结束
    int getExitCode(); // 返回进程的退出码
    void setExitCode(int Code); // 设置进程的退出码
    int waitingProcessSpaceId; // 等待进程的SpaceId
    int waitProcessExitCode; // 等待进程的退出码
    int UserProgramID;
    int exitCode; // 进程的退出码
